...
for i in vocab:
    for e in i:
        if e not in LOWERCASE and e not in LOWERCASE_OTHERS:
            LOWERCASE_OTHERS += [e]
...
# adaptação do método do Norvig: https://norvig.com/spell-correct.html
def candidates(word):
...
def P(word, N=sum(vocab.values())):
    V = 1e5  # quantas palavras existem mas não pegamos - um chute -, usado para smoothing
    count = vocab[word] if word in vocab else 0
    return (count + 1) / (N + V)  # smoothing.
...
def textCorrection(text):
    funcs = [tokenCorrection, punctuationCorrection]
